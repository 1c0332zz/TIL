...
def ranking():
    ...
    results = response.get('results') # 영화 인기순위를 빼내옴
    return sorted(results, key=lambda x: x['vote_average'], reverse = True)[:5] # 딕셔너리 vote_average를 람다로 정렬시킨 후 reverse로 점수가 높은게 앞쪽으로 나오게 바꿔줌 
                                                                                # 인덱스를 이용해 5위까지 출력
